...
var JSONToUVMesh = function(modelJSON, imageID, gl, program, flipTexture, material)
{
	...
	return new UVMesh(gl, program, positionArray, indexArray, normalArray, texCoordArray, material);
}

var JSONtoRGBMesh = function(modelJSON, gl, program, material)
{
	var positionArray = modelJSON.meshes[0].vertices;
	var normalArray = modelJSON.meshes[0].normals;
	var indexArray = [].concat.apply([], modelJSON.meshes[0].faces);

	return new RGBMesh(gl, program, positionArray, indexArray, normalArray, material);
}

var ThreeJSToUVMesh = function(threeJSON, gl, program, material)
{
...
var ThreeJSToRGBMesh = function(threeJSON, gl, program, material)
{
	var indexArray = threeJSON.data.index.array;
	var positionArray = threeJSON.data.attributes.position.array;
	var normalArray = threeJSON.data.attributes.normal.array;
